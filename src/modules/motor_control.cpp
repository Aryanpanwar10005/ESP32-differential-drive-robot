...
void motorInit() {
  ...
  ledc_channel_config_t left_channel = {.gpio_num = LEFT_PWM,
  ...
  ledc_channel_config_t right_channel = {.gpio_num = RIGHT_PWM,
                                         .speed_mode = LEDC_LOW_SPEED_MODE,
                                         .channel =
...
void setMotorSpeeds(int16_t leftSpeed, int16_t rightSpeed) {
  ...
  if (leftSpeed > 0) {
    ...
    digitalWrite(LEFT_IN2, LOW);
    ledc_set_duty(LEDC_LOW_SPEED_MODE, (ledc_channel_t)PWM_CHANNEL_LEFT,
                  leftSpeed);
  } else if (leftSpeed < 0) {
    // Reverse
    digitalWrite(LEFT_IN1, LOW);
  ...
  if (rightSpeed > 0) {
    ...
    digitalWrite(RIGHT_IN2, LOW);
    ledc_set_duty(LEDC_LOW_SPEED_MODE, (ledc_channel_t)PWM_CHANNEL_RIGHT,
                  rightSpeed);
...
void moveForward(int16_t speed) { setMotorSpeeds(speed, speed); }

void moveBackward(int16_t speed) { setMotorSpeeds(-speed, -speed); }

void turnLeft(int16_t speed) { setMotorSpeeds(-speed, speed); }

void turnRight(int16_t speed) { setMotorSpeeds(speed, -speed); }

void brakeMotors() {
  // Active brake: both H-bridge inputs HIGH
  digitalWrite(LEFT_IN1, HIGH);
  digitalWrite(LEFT_IN2, HIGH);
  digitalWrite(RIGHT_IN1, HIGH);
  digitalWrite(RIGHT_IN2, HIGH);

  // PWM MUST BE ZERO during braking (no continuous current)
  ledc_set_duty(LEDC_LOW_SPEED_MODE, (ledc_channel_t)PWM_CHANNEL_LEFT, 0);
  ledc_set_duty(LEDC_LOW_SPEED_MODE, (ledc_channel_t)PWM_CHANNEL_RIGHT, 0);
  ledc_update_duty(LEDC_LOW_SPEED_MODE, (ledc_channel_t)PWM_CHANNEL_LEFT);
  ledc_update_duty(LEDC_LOW_SPEED_MODE, (ledc_channel_t)PWM_CHANNEL_RIGHT);
  ...
  currentLeftSpeed = 0;
  currentRightSpeed = 0;
  Serial.println("[MOTOR] Active braking applied");
}

void coastMotors() {
  // Passive coast: both inputs LOW
  digitalWrite(LEFT_IN1, LOW);
  digitalWrite(LEFT_IN2, LOW);
  digitalWrite(RIGHT_IN1, LOW);
  digitalWrite(RIGHT_IN2, LOW);
